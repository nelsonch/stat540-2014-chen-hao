...
datMin <- data.frame(MinStrain1 = unlist(apply(as.matrix(subset(dat, select = strain1)), 1, min)),
                     MinStrain2 = unlist(apply(as.matrix(subset(dat, select = strain1)), 1, min)))
...
dge.glm.trend.disp.clean <- estimateGLMTrendedDisp(dge.glm.com.disp.clean)
...
```
There are `r nrow(edgeRHits)` which are significatn. Show the first 5 such genes.
```{r}
edgeRHits[1:5, ]
```
...
```
There are `r nrow(DESeqHits)` which are significatn. Show the first 5 such genes.
```{r}
DESeqHits[1:5, ]
```
...
```
There are `r nrow(voomlimmaHits)` which are significatn. Show the first 5 such genes.
```{r}
voomlimmaHits[1:5, ]
```
...
```{r}
allGenes <- unique(c(rownames(edgeRHits), rownames(DESeqHits), rownames(voomlimmaHits)))
geneHits <- data.frame(edgeR = allGenes %in% rownames(edgeRHits),
                       DESeq = allGenes %in% rownames(DESeqHits),
