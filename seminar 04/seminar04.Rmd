...
Convert the above matrix of p-values into a matrix of TURE/FALSE, the cut-off p-value is 0.05. The following table shows the first 10 rows of such a table. 
```{r results = 'asis'}
cutoff <- 0.05
tfRes <- Res
tfRes$t.test <- ifelse(tfRes$t.test < cutoff, TRUE, FALSE)
tfRes$wilcox.test <- ifelse(tfRes$wilcox.test < cutoff, TRUE, FALSE)
tfRes$ks.test <- ifelse(tfRes$ks.test < cutoff, TRUE, FALSE)
...
sumRes <- cbind(gene = tfRes$gene, sumRes, sum.test = testsum)
